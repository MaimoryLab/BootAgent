import { FolderOpen, RefreshCw, RotateCcw, ShoppingBag, Sparkles, Trash2, Upload, X } from "lucide-react";
...
export function SkillsPage() {
  ...
  const importToLibrary = () => {
    ...
    const libraryChanges = imported.map((item) => ({ id: item.id, variant_hash: item.hash, targets: [], import_source: item.source }));
    if (!libraryChanges.length) return;
    setBusy(true);
    void api.applySkills({ preview_token: token, changes: libraryChanges }).then((result) => {
      const failed = (result.results ?? []).filter((item) => item.error);
      setMessage(failed.length ? failed.map((item) => `${item.agent || "Skill"}: ${item.error}`).join("; ") : t("已导入 BootAgent，可在 Skills 页面选择目标 Agent"));
  ...
  return <PageScaffold title={t("Skills")} description={t("管理本机 Skills 并同步到 Agent")} bodyClassName="skills-page" secondaryAction={<div className="management-footer-actions"><Link className="button button-secondary" to="/marketplace?category=agent-enhance"><ShoppingBag size={16} />{t("去市场发现")}</Link><button className="button button-secondary" onClick={scan} disabled={busy} title={t("重新扫描 Skills")}><RefreshCw size={16} />{t("重新扫描 Skills")}</button><button className="button button-secondary" onClick={() => importSource("folder")} disabled={busy} title={t("导入文件夹")}><FolderOpen size={16} />{t("导入文件夹")}</button><button className="button button-secondary" onClick={() => importSource("zip")} disabled={busy} title={t("导入 ZIP")}><Upload size={16} />{t("导入 ZIP")}</button></div>} primaryLabel={dirtyCount ? t("应用 Skills（{count}）", { count: dirtyCount }) : t("应用 Skills")} onPrimary={apply} primaryDisabled={!dirty || busy}>
