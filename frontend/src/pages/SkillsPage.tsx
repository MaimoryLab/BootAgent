import { FolderOpen, RefreshCw, RotateCcw, Sparkles, Trash2, Upload } from "lucide-react";
...
export function SkillsPage() {
  ...
  const changesForApply = () => [...Object.entries(changes).map(([key, change]) => key.includes("::remove::") ? { ...change, id: change.id, delete: true } : change), ...Object.keys(pendingDeletes).map((id) => ({ id, variant_hash: rows.find((row) => row.id === id)?.variant_hashes?.[0] ?? "", targets: rows.find((row) => row.id === id)?.agents ?? [], delete: true }))];
  ...
  const toggleDelete = (id: string) => setPendingDeletes((current) => ({ ...current, [id]: !current[id] }));
  const confirmImport = () => {
