...
export function MCPPage() {
  ...
  return <PageScaffold title={t("MCP 服务器")} description={t("在已初始化的 Agent 之间同步 MCP 服务器")} bodyClassName="mcp-page" secondaryAction={<div className="mcp-footer-actions"><button className="button button-secondary" onClick={refresh} disabled={scanning} title={t("重新扫描")}><RefreshCw size={16} />{t("重新扫描")}</button><button className="button button-secondary" onClick={openNew}><Plus size={16} />{t("新增")}</button></div>} primaryLabel={t("应用更改")} onPrimary={() => void apply()} primaryDisabled={!dirty}>
