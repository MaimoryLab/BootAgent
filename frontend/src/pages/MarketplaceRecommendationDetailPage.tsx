...
export function MarketplaceRecommendationDetailPage() {
  ...
  const returnTo = typeof location.state?.returnTo === "string" ? location.state.returnTo : "/marketplace?recommendationHistory=1";
  ...
  const remove = async () => {
    if (!history) return;
    try { await api.deleteRecommendationHistory(history.id); navigate(returnTo, { replace: true }); }
    catch (error) { setFailure(describeFailure(error, t("无法删除推荐历史"), t).message); }
  };

  if (failure) return <PageScaffold title={t("推荐结果")} onBack={() => navigate(returnTo)}><p className="settings-field-error" role="alert">{failure}</p></PageScaffold>;
  if (!history) return <PageScaffold title={t("推荐结果")} onBack={() => navigate(returnTo)}><EmptyState icon={FileText} title={t("推荐记录不存在")} hint={t("这条记录可能已被删除")} /></PageScaffold>;

  return (
    <PageScaffold title={t("推荐结果")} description={history.need} onBack={() => navigate(returnTo)} backLabel={t("返回推荐历史")} footerNote={<span>{new Date(history.created_at).toLocaleString()} · {history.agent_id} · {history.results?.length ?? 0} {t("个结果")}</span>}>
