...
const english = {
  ...
  "选择卸载实例": "Select installations to uninstall",
  "默认选中全部可卸载实例": "All removable installations are selected by default",
  "继续卸载": "Continue uninstall",
  "可卸载": "Removable",
  "不可卸载": "Not removable",
  "检测到的安装实例": "Detected installations",
  "选择目标 Agent": "Select target Agents",
  "没有可管理目标": "No compatible Agent targets",
  ...
  "导入包含 {count} 个 Skill 冲突：{names}，继续将覆盖现有版本。": "The import contains {count} Skill conflict(s): {names}. Continuing will overwrite existing versions.",
  "是否跳过冲突 Skill 并继续导入？": "Skip conflicting Skills and continue the import?",
  "导入将覆盖已有配置：{names}": "The import will overwrite existing configuration: {names}",
  "导入预览新增：{names}；覆盖：{conflicts}": "Import preview - new: {names}; overwrite: {conflicts}",
  "无": "None",
  ...
  "本地鉴权": "Local authentication",
  "已安全生成": "Generated securely",
  "启动时自动生成": "Generated automatically on start",
  "重新生成 Key": "Regenerate key",
  "无法重新生成本地 Key": "Unable to regenerate the local key",
  "本地协议适配端口不可用": "The local protocol adapter port is unavailable",
  "关闭占用该端口的程序，或在高级设置中更换端口": "Close the program using that port, or choose another port in advanced settings",
  "协议适配端到端验证失败": "Protocol adaptation end-to-end verification failed",
  "检查上游配置模板、模型服务余额和网络后重试": "Check the upstream Profile, model service balance, and network, then retry",
  "协议适配失败且未能完整恢复原配置": "Protocol adaptation failed and the previous configuration could not be fully restored",
  "保留 BootAgent 运行并检查 Claude Desktop 与协议适配设置": "Keep BootAgent running and inspect the Claude Desktop and protocol adapter settings",
  "Claude Desktop 使用基础地址，保存时将调整为 {url}": "Claude Desktop uses the base address. It will be saved as {url}.",
  ...
  "启用协议适配": "Enable protocol adaptation",
  "启用协议适配并应用": "Enable adapter and apply",
  "启用并应用": "Enable and apply",
  "BootAgent 将在本机启动协议适配服务，并自动把 Claude Desktop 指向该服务。是否继续？": "BootAgent will start the local protocol adapter and point Claude Desktop to it automatically. Continue?",
  "正在检查模型服务的协议兼容性": "Checking protocol compatibility",
  "无法检查协议兼容性": "Unable to check protocol compatibility",
  "该模型服务不能直接使用 Claude Desktop 所需的 Anthropic Messages。BootAgent 可以在本机转换请求格式。": "This model service cannot be used directly with the Anthropic Messages API required by Claude Desktop. BootAgent can translate requests locally.",
  "无法确认该配置模版可以通过协议适配使用": "This profile could not be verified for protocol adaptation",
  "让只支持某一种 API 协议的 Agent 也能用上这个模型服务": "Let an Agent that speaks only one API protocol use a model service anyway",
  "将 Responses 与 Messages 请求转换为目标服务支持的 Chat Completions": "Convert Responses and Messages requests into Chat Completions supported by the target service",
  "服务状态": "Service status",
  "选择目标配置后即可启动": "Select a target profile to start",
  "还没有可用的 Chat Completions 配置模板": "No Chat Completions profile is available yet",
  "创建配置模板": "Create profile",
  "适用 Agent": "Compatible Agents",
  "{count} 个可使用": "{count} available",
  "当前没有可使用协议适配的 Agent": "No installed Agent can use protocol adaptation yet",
  "查看环境总览": "View environment overview",
  "了解协议适配": "About protocol adaptation",
  "不同 Agent 使用的 API 协议并不相同。Codex 使用 OpenAI Responses，Claude Code 使用 Anthropic Messages，其他 Agent 可能使用 Chat Completions。": "Agents use different API protocols. Codex uses OpenAI Responses, Claude Code uses Anthropic Messages, and other Agents may use Chat Completions.",
  "BootAgent 只在本机转换请求格式，再把请求发往上方选中的配置模板；模型与凭据仍由该配置模板管理。": "BootAgent converts request formats locally and forwards them to the selected profile; that profile still owns the model and credentials.",
  ...
  "协议适配（Claude Code）": "Protocol adapter (Claude Code)",
  "协议适配（Codex）": "Protocol adapter (Codex)",
  "协议适配（OpenCode / Aider）": "Protocol adapter (OpenCode / Aider)",
  ...
  "卸载并删除已声明数据": "Uninstall and delete declared data",
  "确认永久删除数据": "Confirm permanent data deletion",
  "将永久删除 BootAgent 为该 Agent 声明的数据路径，包括其中的配置、凭据和对话，删除后无法找回。不会额外清理系统钥匙串、声明路径之外的项目目录或自定义路径，也不会删除 BootAgent 的 Profile 和 Provider。所选实例共享这些数据，未选中的实例也可能受影响。请确认你已完成备份。": "This permanently deletes the data paths BootAgent declares for this Agent, including any configuration, credentials, and conversations stored there. This cannot be undone. System keychains, project directories or custom locations outside the declared paths, and BootAgent Profiles and Providers are not removed. These paths are shared across installations, so unselected instances may also be affected. Confirm that you have made a backup.",
  "永久删除并卸载": "Permanently delete and uninstall",
  "卸载中": "Uninstalling",
  "卸载 {name}": "Uninstall {name}",
  "确定卸载「{name}」吗？普通卸载只会移除 Agent 程序，配置模版、模型服务、配置文件和对话数据都会保留。删除已声明数据需要再次确认。": "Uninstall “{name}”? Standard uninstall removes only the Agent program and preserves Profiles, Providers, configuration files, and conversations. Deleting declared data requires another confirmation.",
  "已卸载 {name}，配置和对话数据已保留": "Uninstalled {name}. Configuration and conversation data were preserved.",
  "已卸载 {name}，已清理声明的数据路径": "Uninstalled {name}. Declared data paths were cleaned up.",
  "无法卸载 Agent": "Could not uninstall the agent",
  "运行时": "Runtimes",
  ...
  "MCP 官方 Registry": "Official MCP Registry",
  "npm": "npm",
  "确认跨环境卸载": "Confirm cross-environment uninstall",
  "当前 Agent 由另一套 Node/npm 环境管理。是否允许使用已记录的原始 npm 环境卸载？不会使用 sudo，也不会删除配置和对话数据。": "This Agent is managed by another Node/npm environment. Allow uninstalling it with the recorded original npm environment? sudo will not be used, and configuration and conversations will be kept.",
  "当前 Agent 由另一套 Node/npm 环境管理。是否允许使用已记录的原始 npm 环境卸载？不会使用 sudo。所选实例全部卸载成功后，仍将永久删除已声明的数据；系统钥匙串、声明路径之外的项目目录或自定义路径，以及 BootAgent 的 Profile、Provider 不在清理范围内。": "This Agent is managed by another Node/npm environment. Allow uninstalling it with the recorded original npm environment? sudo will not be used. After all selected instances are uninstalled successfully, the declared data will still be permanently deleted. System keychains, project directories or custom locations outside the declared paths, and BootAgent Profiles and Providers are excluded.",
  "允许并继续": "Allow and continue",
  "Agent 尚未安装": "Agent is not installed",
  "先安装这个 Agent，再执行卸载": "Install this Agent before uninstalling it",
  "Agent 由其他 Node/npm 环境安装": "The Agent was installed by another Node/npm environment",
  "切换到原始 Node/npm 环境后再卸载": "Activate the original Node/npm environment before uninstalling",
  "npm 权限不足": "npm permission denied",
  "检查全局 npm 目录权限，或修复 Node/npm 的用户权限": "Check global npm directory permissions or fix Node/npm user permissions",
  "npm 执行失败": "npm execution failed",
  "检查 npm 环境后重试": "Check the npm environment and try again",
  "PyPI": "PyPI",
  ...
  "描述你的目标，由本地 Agent 从当前目录中筛选合适的工具": "Describe your goal and let a local Agent shortlist tools from the current catalog",
  "只发送你的需求和市场公开目录。本地 Agent 可能连接它已配置的模型服务，但不会获得安装或写文件权限。": "Only your request and the public marketplace catalog are sent. The local Agent may contact its configured model service, but it receives no install or file-write permissions.",
  "正在读取本地 Agent": "Loading local Agents",
  "无法读取本地 Agent": "Unable to load local Agents",
  "没有可用于推荐的本地 Agent": "No supported local Agent is available for recommendations",
  "前往环境总览": "Go to environment overview",
  "用于推荐的 Agent": "Agent used for recommendations",
  "你想完成什么？": "What do you want to accomplish?",
  "例如：整理大型项目的长期知识，并让多个 Agent 复用": "For example: organize long-term project knowledge for reuse across Agents",
  "推荐结果": "Recommendations",
  "推荐工具": "Recommend tools",
  "正在推荐": "Finding tools",
  "无法生成工具推荐": "Unable to generate tool recommendations",
  "推荐结果已生成，但保存历史失败": "Recommendations generated, but saving history failed",
  "推荐历史": "Recommendation history",
  "推荐记录仅保存在本机": "Recommendation history is stored only on this device",
  "无法读取推荐历史": "Unable to load recommendation history",
  "无法删除推荐历史": "Unable to delete recommendation history",
  "无法清空推荐历史": "Unable to clear recommendation history",
  "暂无推荐历史": "No recommendation history",
  "个结果": "results",
  "删除": "Delete",
  "清空历史": "Clear history",
  "推荐记录不存在": "Recommendation record not found",
  "这条记录可能已被删除": "This record may have been deleted",
  "返回推荐历史": "Back to recommendation history",
  "推荐目录版本": "Catalog version",
  "删除记录": "Delete record",
  "查看工具": "View tool",
  "来源未知": "Unknown source",
  "当前工具不可用": "Tool currently unavailable",
  // detail page
