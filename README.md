...
Claude Desktop is detected and launched on macOS and Windows, but its installation remains manual. BootAgent writes and selects a BootAgent-owned 3P profile for direct Anthropic-compatible endpoints; it does not download Claude Desktop, proxy requests, or provide a restore-to-1P action. The selected model ID must belong to the Claude Sonnet, Opus, Haiku, or Fable family.
