...
测试 399 个顶层用例 / 769 顶层加子测试合计
跨语言门禁 16 个文件、61 个顶层用例、247 个逐输入子测试（不含那 61，合计 308）
  config 151 项（配置与 env 文件逐字节）、app 27 项（整个响应 + 全部落盘文件）
  catalog 14、install 14、profile 14（逐字节）、securefs 13（原子写次序）、
  cmd/oneagent 7（退出码，install.sh 与 CI 读的那个契约）、runtime 7
...
- **然后我用那把尺子把一个对的数字改错了。** 核对 parity 计数时看到脚本报 298、文档写 240,直接改了文档——但脚本的 `with subtests` 同时收顶层与子测试,240 指纯子测试,298 − 58 = 240,两个数各自都对。**有了机械计数还不够,口径也得写在数字旁边**,否则下一次核对的人(包括我自己)会拿另一个口径去「修正」它。两处都已标注是哪一个口径。
