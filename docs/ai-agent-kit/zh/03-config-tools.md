...
BootAgent 提供三种配置方式。第一次使用建议选择内置配置；需要多个 Provider 或多个账号时，再选择 CC Switch 等本机工具。
...
CC Switch 是可选的本机配置工具，不是 BootAgent 的必需依赖。请从其官方项目入口获取当前版本，BootAgent 不把它的二进制或安装脚本重新打进启动包。
...
任何配置工具都不能替代合规网络接入。BootAgent 不提供 VPN、代理、节点订阅或“翻墙下载”能力；如果工具或安装源不可达，请返回 [开始使用](./00-start-here.md) 查看手动安装路径。
