...
台账载体为仓库文件 [distribution-channels.md](distribution-channels.md)：每次发布、弃用或撤回都必须同步更新该文件，它也是第 9 节发布门禁中台账条目的核对对象。

`scripts/check_release.py` 会校验台账**自身**的一致性：状态取值合法、SHA-256 为 64 位十六进制、上传人与上传时间非空、撤回条目带时间和原因、`active` 条目不携带撤回字段、同一版本同一文件名跨渠道摘要一致；当产物恰好存在于本地 `release/` 目录时，以产物实际摘要为准做交叉核对。

这道门禁**不检查台账是否漏记**。台账记录的是已上传到渠道的条目，而 `release/` 只是当前机器这次构建的输出目录——在 Linux 上构建时不会有 macOS 产物，换机器或清理 `release/` 后本地产物也会全部消失。因此"台账缺少某个产物"无法与"该平台在别处构建"区分，不能作为报错依据。漏记只能靠发布流程约束，或将来由写台账的工具本身消除。
