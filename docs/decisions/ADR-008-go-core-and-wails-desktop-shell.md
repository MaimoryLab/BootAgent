...
**写锁已落地，形态与最初的设想不同。** 危险不是文件被写坏——每次写入都经独立临时文件与原子 rename，读者永远看到完整的一个版本。危险是**工作被丢弃**：`Install` 与 `Activate` 都是「读现有配置 → 合并 → 写回」，两次交错意味着第二次读发生在第一次写之前，于是前者的结果被静默覆盖。

真正会发生的是 profile 的 `agent_ids`：并发装 4 个 Agent，每个操作各自读 profile、加上自己那一个再写回，实测**丢 3 个**。用户看到的是总览只列出一个已配置 Agent，另外三个显示未配置而它们的配置文件另有说法。

锁挂在 `Service` 上而非包级变量：它属于被写的那个 home，两个指向不同 home 的 Service 没有理由互相阻塞。**读操作刻意不加锁**——原子 rename 保证 `Status` 看不到半写状态，把它也串行化只会让一次安装挡住总览渲染。
