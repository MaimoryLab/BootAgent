...
- 发行工具由 `cmd/bootagent-release` 统一负责；官网只读取 Release 与仓库 JSON。
- Aider 的上游安装流程可以要求本机 Python 3.12，但这是用户选择 Aider 时的外部前置条件，不是 BootAgent 的构建、测试、运行或发行依赖。
...
cmd/bootagent                 headless CLI
cmd/bootagent-release         native package + manifest/checks
cmd/bootagent-rc              latest Agent RC checks
cmd/bootagent-provider-smoke  Provider protocol RC checks
site/                        independent Astro release site
```

（`site/` 于 2026-08-04 迁出到
[MaimoryLab/BootAgent-site](https://github.com/MaimoryLab/BootAgent-site)。上面的布局是本文
验收当时的事实，保留原样。）
...
- `cmd/bootagent-release` 构建 React、Wails desktop 和纯 Go CLI。
...
- `cmd/bootagent-rc verify-agents` 在隔离 HOME/npm prefix 中真实安装锁定 npm Agent，并验证 PATH 和版本。
- `cmd/bootagent-rc adopted` 将 Codex/Claude Code 指向丢弃端口，区分“配置已采用”和“认证未采用”。
- `cmd/bootagent-provider-smoke` 复用 Go Provider client 验证 models、Chat、Responses、Anthropic Messages。
...
| 本地 GUI | `cmd/bootagent-desktop` + Wails |
| headless CLI | `cmd/bootagent` |
| release/build/check/notices | `cmd/bootagent-release` |
| latest Agent RC | `cmd/bootagent-rc verify-agents` |
| Provider RC | `cmd/bootagent-provider-smoke` |
| config adoption RC | `cmd/bootagent-rc adopted` |
...
go build -o bin/bootagent ./cmd/bootagent
...
```text
go run ./cmd/bootagent-rc verify-agents
go run ./cmd/bootagent-rc adopted
go run ./cmd/bootagent-provider-smoke --provider all --timeout 30s
...
用户的 `~/.bootagent`、Agent 配置路径、profile schema、secret 文件和 backup 命名保持兼容。回滚通过发布上一版已验收的原生包完成，不在新包中恢复已删除的旧 runtime。
