...
| 层级 | 入口 | 证明内容 |
| --- | --- | --- |
| RC 安装 | `go run ./cmd/bootagent-rc verify-agents` | 多 Agent 隔离安装、版本和可执行文件 |
| 配置采用 | `go run ./cmd/bootagent-rc adopted` | Codex/Claude Code 是否真正读取丢弃端点配置 |
| Provider | `go run ./cmd/bootagent-provider-smoke` | models、Chat、Responses、Anthropic Messages |
