...
func TestNormalizeDropsIncompatibleTransportFields(t *testing.T) {
	...
	if remote.Command != "" || len(remote.Args) != 0 || remote.Cwd != "" || len(remote.Env) != 0 {
		t.Fatalf("stdio fields retained: %#v", remote)
	}
	stdio, err := Normalize(Spec{Type: "stdio", Command: "npx", URL: "https://example.test", Headers: map[string]string{"Authorization": "secret"}})
	if err != nil {
		t.Fatal(err)
	}
	if stdio.URL != "" || len(stdio.Headers) != 0 {
		t.Fatalf("remote fields retained: %#v", stdio)
	}
}
